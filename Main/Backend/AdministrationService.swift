...
final class AdministrationService {
	...
	func sendTestEmail() async throws {
		try await networkManager.send(
			.v1AdministrationTestEmail,
			context: .userInitiated
		)
	}

	func emailLog() async throws -> [AdministrationEmailDeliveryRecord] {
		try await networkManager.send(.v1AdministrationEmailLog)
	}
...
private extension Endpoint {
	...
	static let v1AdministrationTestEmail = Endpoint("/v1/administration/test-email", method: .post)
	static let v1AdministrationEmailLog = Endpoint("/v1/administration/email-log")
