...
@MainActor
@Observable
final class CreatedTimetableService {
	static let shared = CreatedTimetableService()
	var timetables = Defaults[.createdTimetables]
	private let network = NetworkManager.shared

	func refresh() async throws {
		do {
			try await apply(network.send(Endpoint("/v1/timetables/created")))
	...
	func update(id: UUID, title: String, subjects: [Subject], isSearchable: Bool) async throws {
		...
		Defaults[.createdTimetables] = timetables
	}
	...
	func delete(id: UUID) async throws {
		try Platform.require(Platform.current.allowsCreatedTimetableMutation)
		try await network.send(Endpoint("/v1/timetables/created/\(id.uuidString)", method: .delete))
	...
	private func apply(_ timetables: [TimetableDetailResponse]) {
		...
		Defaults[.createdTimetables] = self.timetables
	}
}
